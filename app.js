    ...
    reader.onload = function(e) {
        try {
            ...
            workbook.SheetNames.forEach(sheetName => {
                ...
                let activityIdx = headers.findIndex(h => h.includes('ativid') || h.includes('nome') || h.includes('process') || h.includes('demand') || h.includes('descric') || h.includes('taref') || h.includes('item'));
                let teamIdx = headers.findIndex(h => h.includes('equip') || h.includes('area') || h.includes('grupo') || h.includes('setor') || h.includes('time') || h.includes('gerenc') || h.includes('diretor'));
                let respIdx = headers.findIndex(h => h.includes('respons') || h.includes('recurs') || h.includes('colaborad') || h.includes('execut') || h.includes('dono') || h.includes('owner') || h.includes('analist') || h.includes('focal') || h.includes('membro') || h.includes('integrant') || h.includes('pesso') || h.includes('operad') || h.includes('atendent'));
                let productIdx = headers.findIndex(h => h.includes('produt') || h.includes('product') || h.includes('prod') || h.includes('servic') || h.includes('sistem') || h.includes('plataform'));
                let volIdx = headers.findIndex(h => h.includes('volum') || h.includes('qtd') || h.includes('quant') || h.includes('vol'));
                let minIdx = headers.findIndex(h => h.includes('tempo') || h.includes('minut') || h.includes('durac') || h.includes('tma') || h.includes('lead') || h.includes('hora'));
                let freqIdx = headers.findIndex(h => h.includes('freq') || h.includes('execuc') || h.includes('periodic'));
                
                // Track claimed column indices to avoid positional fallback collisions
                const claimed = new Set([activityIdx, teamIdx, respIdx, productIdx, volIdx, minIdx, freqIdx].filter(i => i !== -1));
                
                if (activityIdx === -1 && !claimed.has(0) && headers.length > 0) { activityIdx = 0; claimed.add(0); }
