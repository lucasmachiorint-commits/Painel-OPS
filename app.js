    ...
    reader.onload = function(e) {
        try {
            ...
            workbook.SheetNames.forEach(sheetName => {
                ...
                let activityIdx = headers.findIndex(h => h.includes('atividade') || h.includes('nome') || h.includes('processo') || h.includes('demanda') || h.includes('descrição') || h.includes('descricao'));
                let teamIdx = headers.findIndex(h => h.includes('equipe') || h.includes('área') || h.includes('area') || h.includes('grupo') || h.includes('setor') || h.includes('time'));
                let respIdx = headers.findIndex(h => h.includes('responsável') || h.includes('responsavel') || h.includes('dono') || h.includes('colaborador') || h.includes('owner') || h.includes('analista'));
                let productIdx = headers.findIndex(h => h.includes('produto') || h.includes('produtos') || h.includes('product') || h.includes('products') || h.includes('prod') || h.includes('serviço') || h.includes('servico') || h.includes('item'));
                let volIdx = headers.findIndex(h => h.includes('volume') || h.includes('qtd. mês') || h.includes('qtd. mes') || h.includes('quantidade') || h.includes('vol'));
                let minIdx = headers.findIndex(h => h.includes('tempo') || h.includes('minuto') || h.includes('minutos') || h.includes('duração') || h.includes('duracao'));
                ...
                for (let i = 1; i < rows.length; i++) {
                    ...
                    const respName = respIdx !== -1 && row[respIdx] !== undefined && row[respIdx] !== null ? String(row[respIdx]).trim() : '';
