...
async function handleLogin() {
    ...
    try {
        ...
        if (error) {
            ...
            if (user) {
                sessionStorage.setItem('painel_ops_session_active', 'true');
                recordUserActivity();
...
async function handleLogout(isAutoTimeout = false) {
    unsubscribeRealtime();
    sessionStorage.removeItem('painel_ops_session_active');
    ...
    if (client) {
        ...
        try {
            if (isRecoveryHash) {
                ...
                const hasActiveSession = sessionStorage.getItem('painel_ops_session_active') === 'true';
                const lastActivity = parseInt(localStorage.getItem('painel_ops_last_activity') || '0', 10);
            ...
            sessionStorage.removeItem('painel_ops_session_active');
            try {
                localStorage.removeItem('sb-maguyzjhldcgpcvkvkqe-auth-token');
